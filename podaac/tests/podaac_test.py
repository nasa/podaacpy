...
class test_podaac(unittest.TestCase):
	...
	def test_load_dataset_md(self):
		...
		assert_raises(requests.exceptions.HTTPError, self.podaac.load_dataset_md, 'PODAAC-GHMG2-2PO01', 'OSDPD-L2P-MSG02', 'is')
	...
	def test_load_granule_md(self):
		...
		granule_md =  self.podaac.load_granule_md(datasetId, datasetShortName, granuleName)
		root = ET.fromstring(granule_md.encode('utf-8'))
		shortName = root[1][0].attrib

		assert granule_md != None 
		assert str(shortName['id']) == datasetShortName
		assert_raises(requests.exceptions.HTTPError, self.podaac.load_granule_md, format='is')
	...
	def test_load_last24hours_datacasting_granule_md(self):
		...
		format = 'datacasting'
		itemsPerPage = 10
		granule_md = self.podaac.load_last24hours_datacasting_granule_md(datasetId, datasetShortName, format, itemsPerPage)
		root = ET.fromstring(granule_md.encode('utf-8'))
		datasetId_ = root[0][3].text

		assert granule_md != None 
		assert datasetId_ == datasetId
		assert_raises(requests.exceptions.HTTPError, self.podaac.load_last24hours_datacasting_granule_md, 'PODAAC-ASOP2-25X01', 'ASCATA-L2-25km', format='iso')

	#test case for the function search_dataset()
	def test_search_dataset(self):
		format = 'atom'
		itemsPerPage = '400'
		datasets = self.podaac.search_dataset(format=format, itemsPerPage=itemsPerPage)
		...
		assert_raises(requests.exceptions.HTTPError, self.podaac.search_dataset, format='iso')
	...
	def test_search_granule(self):
		...
		startIndex = '1'
		format = 'atom'
		granules = self.podaac.search_granule(datasetId=testDatasetId, startTime=startTime, endTime=endTime, bbox=bbox, startIndex=startIndex, format=format)
		...
		assert_raises(requests.exceptions.HTTPError, self.podaac.search_granule, format='html')
	...
	def test_load_image_granule(self): 
		...
		height = '300'
		width = '200'
		data = self.podaac.load_image_granule(datasetId, shortName, granuleName, bbox, height, width, srs)
		test_data = data[0].split('/')
		length = len(test_data)

		assert data != None 
		assert test_data[length-1] == datasetId+'.png'
		assert_raises(Exception, self.podaac.load_image_granule,datasetId="HBJHKASD")
	...
	def test_extract_granule(self):
		...
		bbox = '45,0,180,90'
		format = 'netcdf'
		data = self.podaac.extract_granule(datasetId, shortName, granuleName, bbox, format)
		test_data = data[0].split('/')
		length =  len(test_data)
